...
bool RTUutils::validCRC(ModbusMessage msg) {
  return validCRC(msg.data(), msg.size() - 2,
                  msg[msg.size() - 2] | (msg[msg.size() - 1] << 8));
}
  ...
  if (!ASCIImode) {
    ...
    while (state != FINISHED) {
      ...
      case WAIT_DATA:
        ...
        break;
      // IN_PACKET: read data until a gap of at least _interval time passed
      // without another byte arriving
      case IN_PACKET:
...
const char RTUutils::ASCIIwrite[] = {0x30, 0x31, 0x32, 0x33, 0x34, 0x35,
